...
model-index:
- name: DeBERTa-v3-Small PII Redaction
  results:
  - task:
    ...
    metrics:
    - type: f1
      value: 0.9517462866318748
    - type: precision
      value: 0.9463893656939842
    - type: recall
      value: 0.9571641971819613
---

# PII Redaction Model

Fine-tuned [microsoft/deberta-v3-small](https://huggingface.co/microsoft/deberta-v3-small) for Named Entity Recognition targeting 27 PII entity types. Trained on the English subset of [ai4privacy/pii-masking-300k](https://huggingface.co/datasets/ai4privacy/pii-masking-300k) with a class-weighted `CrossEntropyLoss`. Achieves **0.9517 macro-F1** on the validation set.
...
```
```output
[{'entity_group': 'BUILDING', 'score': np.float32(0.998517), 'word': '742', 'start': 12, 'end': 16}, {'entity_group': 'STREET', 'score': np.float32(0.99809444), 'word': 'EvergreenTerrace,', 'start': 16, 'end': 35}, {'entity_group': 'CITY', 'score': np.float32(0.9992367), 'word': 'Springfield,', 'start': 35, 'end': 48}, {'entity_group': 'STATE', 'score': np.float32(0.9984706), 'word': 'IL', 'start': 48, 'end': 51}, {'entity_group': 'POSTCODE', 'score': np.float32(0.97415483), 'word': '62704.', 'start': 51, 'end': 58}]
```
...
Two-phase Fine-tuned (frozen backbone → unfrozen) from [`microsoft/deberta-v3-small`](https://huggingface.co/microsoft/deberta-v3-small) using a weighted token-classification trainer and discriminative LRs.
...
| PASS | 0.9666 | 784 |
| PASSPORT | 0.9502 | 1173 |
| POSTCODE | 0.9685 | 954 |
| SECADDRESS | 0.9739 | 440 |
| SEX | 0.9658 | 969 |
| SOCIALNUMBER | 0.9573 | 1285 |
| STATE | 0.9800 | 995 |
| STREET | 0.9759 | 967 |
| TEL | 0.9674 | 991 |
| TIME | 0.9762 | 1825 |
| TITLE | 0.9667 | 906 |
| USERNAME | 0.9636 | 1295 |
